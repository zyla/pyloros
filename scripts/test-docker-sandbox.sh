#!/usr/bin/env bash
#
# test-docker-sandbox.sh — Integration tests for docker-sandbox.sh
#
# Prerequisites: Docker running, cargo build completed.
...
SANDBOX="$SCRIPT_DIR/docker-sandbox.sh"
...
# Create test config: allow only example.com
TEST_DIR="$(mktemp -d)"
